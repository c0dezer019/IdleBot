...
class UserCommands(Cog):
    ...
    async def check(self, ctx: Context, member: Member):
        ...
        if not command.startswith("!"):
            await ctx.reply(
                f"Hey {ctx.message.author.mention}, I'll give you a response but you may not like the "
        ...
        if status != "active":
            await ctx.reply(
                f'I\'m sorry, but {response_as_dict["name"] if member is None else response_as_dict["username"]} is not currently active.'
            )
        else:
            idle_time: str = (
            ...
            response_str: str = (
                f'Last activity for {response_as_dict["name"] if member is None else response_as_dict["username"]} was '
                f"performed {idle_time} ago."
            )

            general: TextChannel = find(
                lambda x: x.name == "general", ctx.guild.text_channels
            )
    ...
    @command(
        aliases=["per"],
        brief=help_lib["bot_performance_brief"],
        help=help_lib["bot_performance_help"],
    )
    ...
    @command(
        aliases=["server"],
        brief=help_lib["server_health_brief"],
        help=help_lib["server_health_help"],
    )
